...
def handle_commas(s: pd.Series) -> pd.Series:
    ...
    return s.astype(str).str.replace(',', '').astype(int)
...
def prep_activity_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # minutes.
    active_minute_cols = ['minutes_lightly_active', 'minutes_fairly_active', 'minutes_very_active']
    df['minutes_active'] = df[active_minute_cols].sum(axis=1)
    df.drop(active_minute_cols, axis=1, inplace=True)

    # Lastly, add the date components.
